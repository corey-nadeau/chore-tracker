...
function ChildDashboard() {
  ...
  const markChoreComplete = async (choreId) => {
    try {
      ...
      toast.success('Great job! Waiting for parent approval ðŸŽ‰');
    } catch (error) {
    ...
    try {
      ...
      if (goalDoc.exists()) {
        ...
        if (goalCompleted) {
          ...
          toast.success(`ðŸŽ‰ GOAL COMPLETED! ${goalData.title} reached! ðŸŽ‰`, { duration: 6000 });
        } else {
          toast.success(`$${amount.toFixed(2)} moved to ${goalData.title}! ðŸŽ¯`);
        }
      }
    } catch (error) {
  ...
  const applyRewardToGoal = async (choreId, goalId) => {
    try {
      ...
      if (goalCompleted) {
        ...
        toast.success(`ðŸŽ‰ GOAL COMPLETED! ${goal?.title} reached! ðŸŽ‰`, { duration: 6000 });
        if (remainderAmount > 0) {
          toast.success(`$${remainderAmount.toFixed(2)} added to your savings bucket! ðŸ’°`, { duration: 4000 });
        }
      } else if (goalId && amountToApply > 0) {
        const goal = currentGoals.find(g => g.id === goalId);
        toast.success(`$${amountToApply.toFixed(2)} added to your ${goal?.title} goal! ðŸŽ¯ðŸ’°`);
      } else {
        toast.success(`$${chore.reward.toFixed(2)} added to your savings bucket! ðŸ’°`);
      }
    } catch (error) {
  ...
  const reassignChore = async (choreId, newGoalId, oldGoalId) => {
    try {
      ...
      if (newGoalId) {
        toast.success(`$${chore.reward.toFixed(2)} moved to ${newGoal?.title} goal! ðŸŽ¯`);
      } else {
        toast.success(`$${chore.reward.toFixed(2)} moved to savings bucket! ðŸª£`);
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-secondary-50">
        <div className="text-center">
          <div className="text-8xl mb-6 animate-bounce">ðŸƒâ€â™‚ï¸</div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-400 via-blue-500 to-green-400 p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-12 text-center max-w-md w-full">
          <div className="text-massive mb-6">ðŸŽ¯</div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50 p-4 pb-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center mb-8">
          <div className="text-massive mb-6 animate-bounce-slow">ï¿½</div>
          ...
          <div className="text-center">
            ...
            {currentGoals.length > 0 ? (
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-purple-800 mb-4">ðŸŽ¯ Your Goals</h3>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="text-purple-400">ðŸ‘† Click to view</div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-3">
                          ...
                          {isCompleted ? (
                            ...
                            <p className="text-purple-600 text-center">
                              ðŸ’ª ${((goal.targetAmount || 0) - (goal.savedAmount || 0)).toFixed(2)} left to reach your goal!
                            </p>
                          )}
                        </div>
                      ) : (
        ...
        <div className="mb-8">
          ...
          {assignedChores.length === 0 ? (
            <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
              ...
              <p className="text-gray-600">No chores assigned right now. Great job! ðŸŒŸ</p>
            </div>
          ) : (
                ...
                <div key={chore.id} className="bg-white rounded-3xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-200">
                  ...
                  <div className="text-center mb-4">
                    ...
                    <div className="flex justify-center items-center space-x-4 text-sm text-gray-500 mb-4">
                      <span>ðŸ“ {chore.location}</span>
                      <span>ðŸ’° ${chore.reward.toFixed(2)}</span>
                      {chore.type && <span>ðŸ  {chore.type}</span>}
                    </div>
                    ...
                    {chore.dueDate && (
                      <p className="text-sm text-orange-600 mb-4">
                        ðŸ“… Due: {new Date(chore.dueDate.toDate()).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center space-x-4 text-sm text-gray-500">
                                <span>ðŸ’° ${chore.reward.toFixed(2)}</span>
                                <span>ðŸ“… {new Date(chore.rewardAppliedAt?.toDate() || chore.approvedAt?.toDate() || chore.createdAt).toLocaleDateString()}</span>
                                <span>
                                  ðŸŽ¯ {isAssignedToThisGoal 
                                    ? 'This goal' 
                            ...
                            <div className="ml-4">
                              ...
                              >
                                <option value="">Savings Bucket ðŸª£</option>
          ...
          >
            <div className="p-6 flex-shrink-0">
              ...
              <div className="text-center mb-6 p-4 bg-gray-50 rounded-xl">
                <div className="text-6xl mb-3">{child.profilePicture || 'ðŸ˜Š'}</div>
            ...
            <div className="p-6 flex-shrink-0 border-t border-gray-200">
              <div className="text-center">
                <p className="text-gray-600 text-sm">
                  Choose from animals, faces, or fun characters! ðŸŽ‰
                </p>
              </div>
            </div>
          </div>
